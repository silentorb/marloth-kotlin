...
fun selectGraph(village: Village, name: String): StateTransform = pipe({ state: State ->
...
fun selectGraph(village: Village): StateTransform =
    ifNotNull({ state: State -> state.gui.activeGraph }) { selectGraph(village, it) }
...
fun renameGraph(change: Renaming): StateTransform = { state ->
  Files.move(
...
//  gui.copy(
//      texturing = gui.texturing.copy(
//          tilePreview = value
//      )
//  )
//}

fun setTilePreview(value: Boolean): StateTransform = { state ->
  state.copy(
      texturing = state.texturing.copy(
          tilePreview = value
      )
  )
}
...
val loadGraphs: StateTransform = { state ->
...
private fun changeDomain(domain: Domain): StateTransform = { state ->
  state.copy(
      domain = domain,
      gui = state.otherDomains.getValue(domain),
      otherDomains = state.otherDomains.minus(domain).plus(state.domain to state.gui)
  )
}

fun selectDomain(village: Village, domain: Domain): StateTransform =
...
fun updateState(village: Village, focus: FocusContext, event: Event, undo: StateTransform, redo: StateTransform): StateTransform =
    when (event.type) {
      ...
      EventType.textureSelect -> selectGraph(village, event.data as String)
      ...
      EventType.switchDomain -> selectDomain(village, event.data as Domain)
      EventType.undo -> undo
      EventType.refresh -> selectGraph(village)
      EventType.renameTexture -> renameGraph(event.data as Renaming)
    }