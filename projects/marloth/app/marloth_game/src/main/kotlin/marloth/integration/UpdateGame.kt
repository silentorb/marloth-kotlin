...
fun updateWorld(app: GameApp, state: AppState): List<World> {
  ...
  val nextWorld = simulation.updateWorld(app.client.renderer.animationDurations, world, commands, simulationDelta)
...
fun updateFixedInterval(app: GameApp, boxes: Boxes, newWorld: () -> World): (AppState) -> AppState = { state ->
  ...
  val worlds = when {
    nextClientState.commands.any { it.type == GuiCommandType.newGame } -> listOf(newWorld())
