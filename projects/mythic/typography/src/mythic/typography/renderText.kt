...
private val unit_conversion = 24f
...
fun prepareText(config: TextConfiguration) {
  ...
  val element_count = without_spaces.length
  if (element_count == 0) {
    return
  }
  ...
  val letter_space = 6f
  val max_width = _max_width * unit_conversion / size
  ...
  for (i in 0 until content.length) {
    ...
    vertices.set(step + 5, Vector4(x, py, 0f, character.offset + character.height))
    vertices.set(step + 4, Vector4(x, py - height, 0f, character.offset))
    vertices.set(step + 3, Vector4(x + width, py - height, texture_width, character.offset))
    vertices.set(step + 2, Vector4(x + width, py, texture_width, character.offset + character.height))
...
fun renderText(font: Font) {
  val scale = get_scale()
  val scaling = effect.get_viewport().get_unit_scaling()

  val transform = glm.translate(mat4(1), vec3(position.x * scaling.x, position.y * scaling.y, 0)) * glm.scale(mat4(1), vec3(scale * scaling.x, scale * scaling.y, 1))
